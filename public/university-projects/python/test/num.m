...
%   num("__show__")          % displays bucket sizes found in workspace
%   num("__clear__")         % clears num* buckets created by this tool

function final = num(word)

  % --- utility commands (optional) ---
  if (nargin == 1) && ischar(word)
    cmd = strtrim(word);
    if strcmpi(cmd, "__show__")
      show_buckets_();
      final = NaN;
      return;
    elseif strcmpi(cmd, "__clear__")
      clear_buckets_();
  ...
  if ~ischar(word)
    error('Input must be text, e.g., num("Hello") or num(''Hello'')');
  endif
