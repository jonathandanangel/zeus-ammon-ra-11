...
import { loadAiDetectorPolarity } from "@/game/ai-detector/polarity";
import {
  DETECTORS,
  toneForBand,
  type DetectorId,
...
export function AiDetectorApp({ onMenu }: { onMenu: () => void }) {
  ...
  // Polarity commands stay active (default flipped); no user switch — tuned over time in code.
  const polarity = loadAiDetectorPolarity();
        ...
        {showInfo && (
          <div className="space-y-2 border-b border-cyan/20 px-4 py-3 font-mono text-[11px] leading-relaxed text-muted-foreground">
            ...
            <p className="text-mint/90">
              Improving over time: new reasoning parameters, human-band rules (convergent /
              balanced), and fusion commands are integrated manually into this Free ensemble —
              same detectors, tighter judgment. Library of Babel polish uses the same stack.
            </p>
            <p className="text-amber/90">
              Tip: you might need shorter texts for more accuracy — paste a focused passage (~50–300
              words) instead of a whole chapter when you want a cleaner read.
            </p>
            <p className="text-amber/90">
